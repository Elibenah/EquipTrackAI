...
using SimpleSockets;
using TikshuvServer.connections;
using TikshuvServer.signatureManagment;
...
namespace TikshuvServer
{
    public class Server
    {
        ...
        public static void SetupServer(string Name,string ip, int port)
        {
            _encrypt = false;
            _compress = true;
        ...
        {
            WriteLine("Server received a MessageContract from the client with id " + clientInfo.Id + " the header is : " + header);
        }
        ...
        public static IDictionary<int, IClientInfo> getClientList()
        {
            return _listener.GetConnectedClients();
        }
        ...
        private static async void SendObject(IClientInfo client,responseObject responseToClient)
        {
            ...
            await _listener.SendObjectAsync(client.Id, responseToClient,true,false,false);
        }
        ...
        {
            requestObject requestFromClient = new requestObject();

            string x = obj.GetType().Name;
            string jsonRequest = obj.ToString();
            requestFromClient = JObject.Parse(jsonRequest).ToObject<requestObject>();

            responseObject responseToClient = new responseObject();
            responseToClient = MessageHandler(requestFromClient);
            WriteLine("A message has been sent to client " + client.Id + " response to: "+ requestFromClient.request);
        ...
        private static responseObject MessageHandler(requestObject requestFromClient)
        {
            ...
            responseToClient.request = requestFromClient.request;
            responseToClient.requestSubject = requestFromClient.requestSubject;

            if (requestFromClient.requestSubject.Equals("solider"))
            {
                array = soliderRequestHandling.soliderRequestFromClient(requestFromClient);
                responseToClient.data = array[0];
                responseToClient.authorized = Convert.ToBoolean(array[1]);
            }
            else if (requestFromClient.requestSubject.Equals("equipment"))
            {
                array = equipmentRequestHandling.equipmentRequestFromClient(requestFromClient);
                responseToClient.data = array[0];
                responseToClient.authorized = Convert.ToBoolean(array[1]);
            }
            else if (requestFromClient.requestSubject.Equals("mission"))
            {
                array = missionRequestHandling.equipmentRequestFromClient(requestFromClient);
                responseToClient.data = array[0];
                responseToClient.authorized = Convert.ToBoolean(array[1]);
            }
            else if (requestFromClient.requestSubject.Equals("signature"))
            {
                array = signatureRequestHandling.signatureRequestFromClient(requestFromClient);
                responseToClient.data = array[0];
                responseToClient.authorized = Convert.ToBoolean(array[1]);
            }
        ...
        public static void broadcastUpdate(string requestSubject,string request,object data)
        {
            responseObject responseToClient = new responseObject();
            responseToClient.authorized = true;
            responseToClient.requestSubject = requestSubject;
            responseToClient.request = request;
            responseToClient.data = data;
            Server.sendToAllClients(responseToClient);
        }
        public static void sendToAllClients(responseObject response)
        {
            Task taskA = Task.Run(() => 
            {
                IDictionary<int,IClientInfo> clients = new Dictionary<int,IClientInfo>();
                clients = getClientList();

                foreach (var client in clients.Values)
                {
                    SendObject(client,response);
                }
                WriteLine("ended broadcasting");
            });
        }
